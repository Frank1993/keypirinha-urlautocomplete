...
class GoogleTranslate(kp.Plugin):
    ...
    def _complete_cosmos_url(self, url, cluster, vc):
        cluster = "cosmos08"
        vc = "Bingads.algo.prod.IntentMatching"
    ...
    def _create_autocomplete_item(self, vc, finalurl):
        vc = "IntentMatching"
    ...
    def on_suggest(self, user_input, items_chain):
        ...
        query = self._complete_cosmos_url(user_input,None,None)

        suggestions.append(self._create_result_item_cosmos(None,None,None,None))
    ...
    def _create_result_item_cosmos(self, lang_in, lang_out,
                            search_terms, search_result):
        short_desc = "autocomplete"
        item = self.create_item(
            category=self.ITEMCAT_RESULT,
            label=search_result if search_result else "intentmatching",
            short_desc=short_desc,
            target=search_result if search_result else "intentmatching",
            args_hint=kp.ItemArgsHint.FORBIDDEN,
            hit_hint=kp.ItemHitHint.IGNORE)
